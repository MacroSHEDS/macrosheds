...
#' @param precip \code{date.frame} or file path to feather file.
#'    A dataset in MacroSheds format (see PLACEHOLDER0 for definitions),
#'    containing precipitation gauge data. A \code{tibble} in
...
#'    If FALSE, the resulting watershed average will be a mean of two separate estimates: the
#'    mean of cell values and the prediction of a linear regression with regression. Note that these
#'    computations are performed at every time step, using all available gauges. At least 3 gauges
#'    must record at a given time step for the regression to be used in prediction.
#'    
#'    If calculating
#'    watershed-average precipitation volume, it makes sense to leverage 
#'    
#'    If only \code{precip} is supplied, only
#'    precipitation volume is 
#'    
#'    ms_calc_watershed_precip will interpolate only precipitation volume if no
