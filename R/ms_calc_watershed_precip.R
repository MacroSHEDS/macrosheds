...
#'
#' Using precipitation gauge recordings (volume and/or chemistry) and locations,
#' this function interpolates an estimated value at each cell of a supplied digital
...
#' elevation and precipitation volume may also be leveraged for better prediction.
#' If precipitation chemistry is supplied in addition to volume, then watershed-average
#' precipitation volume, chemistry, and chemical flux estimates will all be generated.
#' 
#' @author Spencer Rhea, \email{spencerrhea41@gmail.com}
#' @author Mike Vlah
#' @author Wes Slaughter
#' @param precip optional \code{date.frame} or file path to feather file.
#'    A dataset in MacroSheds format (see PLACEHOLDER0 for definitions),
#'    containing precipitation volume time-series.  A \code{tibble} in
#'    MacroSheds format can be generated by [ms_load_product()]. If \code{precip} is
#'    supplied and \code{pchem} is not, only precip
#' @param pchem optional \code{data.frame} or path to a folder containing feather
#'    files of precipitation chemistry data in MacroSheds format (see PLACEHOLDER0).
#' @param ws_boundary \code{sf} object or path to a folder containing shapefiles 
#'    of watershed boundaries. Object/files must have site_code and geometry features (columns).
#'    See [ms_delineate_watershed()].
#' @param precip_gauge \code{sf} object or path to a folder containing shapefiles
...
#' @param parallel logical. If TRUE (default), interpolation uses \code{maxcores} CPU cores/threads.
#' @param maxcores numeric. Default \code{Inf}; the maximum number of cores used in parallel processing, 
#'    ignored if \code{parallel} is FALSE.
...
#' @return Saves watershed-average precipitation volume, chemistry, and chemical flux (if both precip 
#'    and pchem are supplied) to \code{out_path} as feather files in MacroSheds format (see PLACEHOLDER0).
#'    See details regarding \code{elevation_agnostic}
...
#'    mean of cell values and the prediction of a linear regression with elevation Note that these
#'    computations are performed at every time step, using all available gauges. At least 3 gauges
#'    must record at a given time step for the regression to be leveraged in generating the final prediction.
#'    
#'    This function will interpolate only precipitation volume if no
...
#'    
#'    In general, elevation is a relevant predictor for precipitation volume, but not
#'    concentration of precip chemical components. As such, \code{elevation_agnostic}
#'    is ignored when determining watershed-average precipitation chemistry in the case
#'    of both \code{precip} and \code{pchem} being supplied. If only \code{pchem} is
#'    supplied and \code{elevation_agnostic} is TRUE, then the elevation-pchem relationship
#'    \emph{will} be incorporated into the final estimate of watershed-average pchem
#'    for each time step.
#'    
