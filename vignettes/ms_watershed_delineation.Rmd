---
title: Watershed dealineation with the macrosheds package 
author: Mike Vlah
...
To delineate a watershed with no prior knowledge of its area, road density, etc., you 
don't need to supply many arguments. Just latitude, longitude, and their coordinate 
reference system. If you don't know the CRS, try 4326 (WGS 84). In this case, we are 
providing specifications, and setting *verbose* and *confirm* to FALSE, because
this allows the function to run without interaction - necessary for this vignette's R code 
to run and knit into a markdown document without user interaction. In real application, these
specifications should only be provided if the user has already visually inspected each parameter
and knows they approve of the resulting delineation. The reader is encouraged to run this code without 
the specification arguments in their own R console, and see the interactive features. The meaning of the specification arguments 

If you run this as-is, it'll create a new folder called 'my_watershed_boundaries' in your home directory. Feel
free to change that.
...
```{r demo, eval = FALSE}
    out <- ms_delineate_watershed(
        ...
        # arguments below allow for this function
        # to run without interactive confirmation
